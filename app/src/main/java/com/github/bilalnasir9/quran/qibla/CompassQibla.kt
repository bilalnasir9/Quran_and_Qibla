package com.github.bilalnasir9.quran.qibla
...
import com.github.bilalnasir9.quran.ui.fragments.QiblaFragment
import com.github.bilalnasir9.quran.qibla.LocationUtils.checkLocationPermission
import com.google.android.gms.location.FusedLocationProviderClient
import com.google.android.gms.location.LocationServices

class CompassQibla {

    class Builder(private val activity: QiblaFragment, private val model: CompassQiblaViewModel) : SensorEventListener {
        private val TAG: String = "CompassQibla"
...
//        private val model: CompassQiblaViewModel = ViewModelProvider(activity).get(CompassQiblaViewModel::class.java)

        fun build() {
            fusedLocationClient = LocationServices.getFusedLocationProviderClient(activity.requireContext())
            activity.requireContext().checkLocationPermission(onGranted = {
            ...
            fusedLocationClient.lastLocation.addOnSuccessListener {
                it?.let { location ->
                    ...
                    try {
                        model.getLocationAddress(activity.requireContext(), currentLocation)
                    }catch (e: Exception){}
                    try {
                        sensorManager =
                                activity.requireContext().getSystemService(AppCompatActivity.SENSOR_SERVICE) as SensorManager
        ...
        fun onGetLocationAddress(onGetLocation: (address: Address) -> Unit) = apply {
            try {
                model.locationAddress.observe(activity) { it?.let { it1 -> onGetLocation(it1) } }
            }catch (e:Exception){}
        }
